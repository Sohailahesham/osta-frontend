...
export default function UserRegisterPage() {
  ...
  const [form, setForm] = useState<UserRegisterForm>({
    fullName: '', email: '', phone: '', governorate: '',
    city: '', password: '', confirmPassword: '', agreeToTerms: false,
  });
  const [errors, setErrors] = useState<Partial<Record<keyof UserRegisterForm, string>>>({});
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setFailMessage(Array.isArray(message) ? message[0] : message || 'حدث خطأ، حاول مرة أخرى');
      setShowFail(true);
    }
  };
    ...
    <div className="min-h-screen flex relative overflow-hidden" dir="ltr">
      ...
      <div className="flex items-center justify-center z-10 w-full min-h-screen px-4 py-20 lg:w-[55%] lg:px-12 lg:py-8">
        <div className="bg-white rounded-3xl shadow-sm w-full p-7 max-w-sm sm:p-10 sm:max-w-md lg:p-12 lg:max-w-2xl" dir="rtl">
          ...
          <div className="flex flex-col gap-4">
            <AuthInput label="الاسم بالكامل" placeholder="ادخل الاسم الرباعي" type="text"
              value={form.fullName} onChange={update('fullName')} error={errors.fullName} />

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <AuthInput label="البريد الإلكتروني" placeholder="Ex: user@example.com" type="email"
                value={form.email} onChange={update('email')} error={errors.email} />
              <AuthInput label="رقم الهاتف" placeholder="+00 0000 000000" type="tel" icon={Phone}
                value={form.phone} onChange={update('phone')} error={errors.phone} />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <AuthSelect label="المحافظة" placeholder="اختر محافظتك" options={GOVERNORATES}
                value={form.governorate}
                onChange={(val) => { setForm(prev => ({ ...prev, governorate: val, city: '' })); }}
                error={errors.governorate} />
              <AuthInput label="المدينة" placeholder="اكتب مدينتك"
                value={form.city} onChange={update('city')} error={errors.city} disabled={!form.governorate} />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <AuthInput label="كلمة المرور" placeholder="ادخل كلمة المرور" type="password"
                value={form.password} onChange={update('password')} error={errors.password} />
              <AuthInput label="تأكيد كلمة المرور" placeholder="أعد إدخال كلمة المرور" type="password"
            ...
            <div className="flex flex-col gap-1">
              ...
              {errors.agreeToTerms && <span className="text-xs text-red-500 mr-6">{errors.agreeToTerms}</span>}
            </div>
          </div>

          <Button fullWidth onClick={handleSubmit} className='mt-6'>إنشاء حساب</Button>
          ...
          <button className="w-full py-2.5 rounded-full border border-gray-200 flex items-center justify-center gap-2 bg-[#F1F7E7] hover:bg-[#E7F1D8] transition-all">
            ...
            <span className="text-sm text-gray-600 font-medium">المتابعة عبر جوجل</span>
          </button>
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-sm p-8 flex flex-col items-center text-center" dir="rtl">
            ...
            <h2 className="text-xl font-bold text-[var(--primary-color)] mb-2">تم إنشاء حسابك بنجاح</h2>
            <p className="text-gray-400 text-sm mb-8">يمكنك الآن الوصول إلى جميع خدمات أُسطى</p>
            <Button fullWidth onClick={() => router.push('/home')}>الذهاب إلى الصفحة الرئيسية</Button>
          </div>
        </div>
      )}
      ...
      {showFail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-sm p-8 flex flex-col items-center text-center" dir="rtl">
            ...
            <h2 className="text-xl font-bold text-[var(--primary-color)] mb-2">فشل إنشاء الحساب</h2>
            <p className="text-gray-400 text-sm mb-8">{failMessage}</p>
            <Button fullWidth onClick={() => setShowFail(false)}>حاول مرة أخرى</Button>
          </div>
        </div>
      )}
    </div>
