...
import { RefreshCw, Bell, ArrowDownLeft, ArrowUpRight, Wallet } from "lucide-react";
...
export default function WalletPage() {
  ...
  return (
    <div dir="rtl" className="max-w-3xl mx-auto ">
      ...
      <div className="flex items-center justify-between rounded-t-2xl border border-b-0 border-gray-100  px-6 py-5">
        <div className =" flex  ">
            <span
              className="flex h-10 w-10 me-2 items-center justify-center rounded-full bg-[#DEEEE1]"
              
            >
              <Wallet size={14} style={{ color: "var(--primary-color)" }} />
            </span>
         <div>
           <h3 className="flex items-center gap-2 text-lg font-extrabold" style={{ color: "var(--primary-color)" }}>
            المحفظة
          </h3>
          <p className="text-xs mt-0.5 " style={{ color: "var(--gray-color)" }}>
  ...
  <span className="text-xs font-bold text-white/70">
    الرصيد الحالي
        ...
        {/* Info banner */}
        <div
          className="mt-6 flex items-center  bg-gray-100 gap-3 rounded-2xl border border-dashed px-4 py-3"
        ...
        >
          ...
          <p className="text-xs leading-5" style={{ color: "var(--gray-color)" }}>
            ستظهر إشعارات داخل التطبيق فور الموافقة على طلب السحب أو رفضه، خلال مدة لا تتجاوز 48 ساعة عمل
          </p>
        </div>
      </div>
      ...
      {!error && wallet && wallet.transactions.length > 0 && (
        <div className="mt-6">
          <h4 className="mb-3 text-sm font-bold" style={{ color: "var(--gray-color)" }}>
            آخر العمليات
          </h4>
                ...
                >
                  <div className="flex items-center gap-3">
                    <span
                      ...
                      style={{
                        backgroundColor: tx.type === "credit" ? "var(--purple-light)" : "#FEE2E2",
                      }}
                    >
                      {tx.type === "credit" ? (
                        <ArrowDownLeft className="h-4 w-4" style={{ color: "var(--purple-dark)" }} />
                      ) : (
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-[11px] text-gray-400">
                        {formatDate(tx.createdAt)} · {STATUS_LABELS[tx.status] ?? tx.status}
                      </p>
                    </div>
                  </div>
                  <span
                    className="text-sm font-bold shrink-0"
                    style={{ color: tx.type === "credit" ? "var(--purple-dark)" : "#EF4444" }}
                  >
                    {tx.type === "credit" ? "+" : "-"}
